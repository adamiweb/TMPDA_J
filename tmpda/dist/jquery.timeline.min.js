...
if(t.image)
  {
    ...
    s.forEach(function(val,index){
        if(val!=="")
        {
          index==0?i=o("<a />").attr("href",val).append(o("<img />").attr({"src":val,"alt":"Event Photos","title":g.attr("id").substr(g.attr("id").length-4,4)}).addClass("timeline-item-image timeline-item")):i=o("<a />").attr("href",val);
          g.append(i);
          alert(g.attr("id").substr(g.attr("id").length-4,4)+".....14");
        }
    });
  }
